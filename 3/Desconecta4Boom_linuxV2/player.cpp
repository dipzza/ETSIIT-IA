...
double Poda_AlfaBeta(const Environment &t, int jug, int prof, int Limite, Environment::ActionType &accion, double alpha, double beta)
{
	...
	{
		double aux = Poda_AlfaBeta(v[i], jug, prof+1, Limite, accion, alpha, beta);
		...
		if (t.JugadorActivo() == jug)
		{
			if (aux > alpha)
			{
				alpha = aux;
				if (prof == 0)
					accion = static_cast<Environment::ActionType>(v[i].Last_Action(jug));
			}
		}
		else
		{
